    ...
    odometry_bridge_ =
        std::make_unique<Ros2TopicBridge<kachaka_api::GetRosOdometryResponse,
            ...
            std::bind(&kachaka_api::KachakaApi::Stub::GetRosOdometry, *stub_,
                      _1, _2, _3),
            "~/odometry", qos);
    odometry_bridge_->SetConverter([](const kachaka_api::GetRosOdometryResponse&
                                          grpc_msg,
                                      nav_msgs::msg::Odometry* ros2_msg) {
      const auto& odometry = grpc_msg.odometry();
      converter::ConvertGrpcHeaderToRos2Header(odometry.header(),
                                               &ros2_msg->header);
      ...
      ros2_msg->pose.pose.orientation.x =
          odometry.pose().pose().orientation().x();
      ros2_msg->pose.pose.orientation.y =
          odometry.pose().pose().orientation().y();
      ros2_msg->pose.pose.orientation.z =
          odometry.pose().pose().orientation().z();
      ros2_msg->pose.pose.orientation.w =
