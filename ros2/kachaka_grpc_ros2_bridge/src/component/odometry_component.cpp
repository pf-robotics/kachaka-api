...
class OdometryComponent : public rclcpp::Node {
 public:
  explicit OdometryComponent(const rclcpp::NodeOptions& options)
    ...
    if (declare_parameter("publish_wheel_odometry", false)) {
  ...
  ~OdometryComponent() override {
    odometry_bridge_->StopAsync();
    if (wheel_odometry_bridge_) {
      wheel_odometry_bridge_->StopAsync();
    }
  }
