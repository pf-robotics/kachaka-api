...
class DiagnosticsComponent : public rclcpp::Node {
 public:
  explicit DiagnosticsComponent(const rclcpp::NodeOptions& options)
      : Node("diagnostic", options) {
    stub_ = GetSharedStub(declare_parameter("server_uri", ""));

    rclcpp::SensorDataQoS qos;
    using namespace std::placeholders;
    get_robot_error_code_json_bridge_ = std::make_unique<
        GrpcBridge<kachaka_api::EmptyRequest,
    ...
    diagnostic_bridge_ = std::make_unique<
        Ros2TopicBridge<kachaka_api::GetErrorResponse, diagnostic_msgs::msg::DiagnosticArray>>(
        this,
        std::bind(&kachaka_api::KachakaApi::Stub::GetError, *stub_, _1, _2,
                  _3),
        "~/diagnostics", qos);
    diagnostic_bridge_->SetConverter([&, this](const kachaka_api::GetErrorResponse& grpc_msg,
      ...
      for (auto& ec : error_codes) {
        ...
        if (this->robot_error_dictionary_.contains(ec_str)) {
          ...
          } else if (error_info[this->ERROR_TYPE] == this->ERROR_TYPE_WARN) {
            diag.level = diagnostic_msgs::msg::DiagnosticStatus::WARN;
          } else {
            // ERROR_TYPE_ERROR, ERROR_TYPE_FATAL ERROR_TYPE_CALL_FOR_SUPPORT
          ...
          diagnostic_msgs::msg::KeyValue code, title, description, error_type, ref_url;
          code.key = this->CODE;
          code.value = error_info[this->CODE].get<std::string>();
          ...
          error_type.value = error_info[this->ERROR_TYPE].get<std::string>();
